"""Evaluate SeCo / TeCo / CACo pretrained encoders on UC Merced Land Use.

Identical linear-probing pipeline to eval_eurosat_all.py — only the dataset
and num_classes change (21 vs 10). UC Merced is 256x256 1-ft aerial imagery,
far higher resolution than the Sentinel-2 pretraining data — this doubles as
a resolution-generalization stress test that the CACo paper explicitly flags
as a limitation of their representation.

All artifacts saved under CACo/eval_results/ucmerced/.
"""
import json
import sys
import time
...
from datasets.ucmerced_dataset import UCMercedDataset  # noqa: E402

DATA_DIR = ROOT / 'data' / 'UCMerced_LandUse'
OUT_DIR = ROOT / 'eval_results' / 'ucmerced'
...
def get_loader(split, shuffle=False):
    # UCM is nominally 256x256 but ~2% of images are off-by-a-few pixels
    # (242–257 in either dim). Resize to 256x256 to allow batch collation.
    # Pipeline otherwise identical to main_eurosat.py (just ToTensor).
    transform = T.Compose([T.Resize((256, 256)), T.ToTensor()])
    ds = UCMercedDataset(str(DATA_DIR), split=split, transform=transform, get_path=True)
